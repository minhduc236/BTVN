...
IF OBJECT_ID('f_DemDeAnNV') is not null
	drop function f_DemDeAnNV
go
CREATE FUNCTION f_DemDeAnNV(@MaNV varchar(9))
returns int
as
	begin
...
print ' so du an nhan vien da lam la: ' + convert(varchar, dbo.f_DemDeAnNV('004'))
...
create function f_demnv_phai(@Phai nvarchar(5) = N'%')
returns int
as 
	begin
		return(select count(*) 
...
print 'nhan vien nam: ' + convert(varchar, dbo.f_demnv_phai('Nam'))
print 'nhan vien nu: ' + convert(varchar, dbo.f_demnv_phai(N'Nữ'))

--1d---

if OBJECT_ID('f_Luong_NhanVien_PB')is not null
	drop function f_Luong_NhanVien_PB 
go
create function f_Luong_NhanVien_PB(@TenPhongBan nvarchar(20))
...
select * from dbo.f_Luong_NhanVien_PB('IT')

-----1e------

if object_id('f_soLuongDeAnTheoPB') is not null
	drop function f_soLuongDeAnTheoPB
	go
CREATE FUNCTION f_soLuongDeAnTheoPB(@MaPB int)
returns @ListPB table(TenPB nvarchar(20), MATP nvarchar(10), TenTP nvarchar(50), soLuong int)
as 
begin
	insert into @ListPB
...
select * from dbo.f_soLuongDeAnTheoPB(1)
